#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import argparse, csv, time, math, os, requests, pandas as pd
...
def get_all_usdt_spot_symbols():
    ...
    for s in r.json()["symbols"]:
        if s.get("status")=="TRADING" and s.get("quoteAsset")=="USDT" and s.get("isSpotTradingAllowed",False):
            name = s["symbol"]
            if any(x in name for x in ["UPUSDT","DOWNUSDT","BULLUSDT","BEARUSDT"]): continue
            syms.append(name)
    return sorted(syms)

def get_klines_weekly(symbol, limit=180):
    r = requests.get(f"{BINANCE_API}/api/v3/klines",
                     params={"symbol":symbol,"interval":"1w","limit":limit}, timeout=30)
    r.raise_for_status()
    arr = r.json()
...
def screen(df, min_qv, min_price):
    ...
    if math.isnan(avg_qv6) or avg_qv6 < min_qv or last_close < min_price: return None
    m, s, h = macd(df["close"])
    ...
    if (cut_up or on_top_small) and (m_now < 0) and (h_now > 0):
        ...
        return {"close":round(last_close,8),"avg_qv_6w":round(avg_qv6,2),
                "macd":round(m_now,6),"signal":round(s_now,6),"hist":round(h_now,6),
                "score":round(score,3)}
    return None
    ...
    try:
        with open(file_path,"rb") as f:
            r = requests.post(f"https://api.telegram.org/bot{token}/sendDocument",
                              ...
                              files={"document":(os.path.basename(file_path),f,"text/csv")}, timeout=30)
            r.raise_for_status()
        print("Telegram: sent.")
...
def main():
    ...
    for i,sym in enumerate(syms,1):
        try:
            ...
            if rec: results.append({"symbol":sym, **rec})
        except Exception: pass
        if i % 20 == 0: print(f"scanned {i}/{len(syms)}")
